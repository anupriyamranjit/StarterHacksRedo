...
def index(request):
    ...
    if (lengthofEvents > NumberofEvents.objects.get(id=1).number):
    	...
    	for i in range(difference):
    		send_mail(event_details['events'][i]['user']['_id'], event_details['events'][i]['user']['_id']+ ' ' + event_details['events'][i]['type'] + ' ' + event_details['events'][i]['geofence']['description'],'anupriyamranjit8@gmail.com',['nafap19986@topmail1.net'],fail_silently=False)
    else:
    	pass
